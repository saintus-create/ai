  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-4 end-4 z-20 rounded-2xl border bg-fd-card px-4 py-2 text-sm shadow-lg"
      >
        Ask AI
      </button>
      {open ? (
          ...
          <section className="flex max-h-[80vh] w-full max-w-md flex-col rounded-2xl border bg-fd-card p-3 shadow-2xl">
            ...
            <form onSubmit={(event) => { event.preventDefault(); void ask(); }} className="flex gap-2">
              <input
                ...
                disabled={busy}
                placeholder={busy ? 'AI is answering…' : 'Ask a question…'}
                className="min-w-0 flex-1 rounded-xl border bg-transparent px-3 py-2 text-sm outline-none"
              />
